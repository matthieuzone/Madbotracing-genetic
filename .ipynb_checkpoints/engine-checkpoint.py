import numpy as np


maxTurn = 18*2*np.pi/360
Ndt = 20

def u(theta):
    return np.array((np.cos(theta), np.sin(theta)))

def dir(v):
    if np.linalg.norm(v) == 0:
        return 0
    v = v / np.linalg.norm(v)
    if v[1] == 0:
        return -(v[0]/abs(v[0]) - 1)*np.pi/2
    return np.arccos(v[0]) * v[1]/abs(v[1])

def mod(theta):
    return ((theta + np.pi)  % (2*np.pi)) - np.pi


def default_strategy(self):
    return self.game.checkpoints[self.nextchx], 100

class Pod:

    nb = 0
    
    def __init__(self, game, strategy = default_strategy):
        
        self.id = Pod.nb
        Pod.nb += 1
        self.game = game
        
        self.pos = np.array([0.,0.])
        self.v = np.array([0.,0.])
        self.orientation = 0
        self.nextchx = 0
        self.turns = 0
        self.strategy = strategy
        self.boost = True

    def dist(self,p):
        return np.linalg.norm(p - self.pos)

    def timestep(self, action = None):

        if action == None:
            action = self.strategy(self)
        aim, thrust = action

        #end of previous timestemp
        
        self.v *= 0.85
        self.pos = np.array(self.pos).round()
        self.v = np.trunc(self.v)
        
        #start of new tmestep
        turn = mod(dir(aim - self.pos) - self.orientation)
        turn = max(turn, -maxTurn)
        turn = min(turn, maxTurn)
#        if "BOOST" == thrust:
#            if self.boost:
#                thrust = 650
#            else:
#                thrust = 100
#            self.boost = False
#        else:
        assert 0 <= thrust <= 100
        self.orientation += turn
        self.v += thrust*u(self.orientation)
                
    def move(self):
        self.pos += self.v/Ndt
        self.checkCollide()

        if self.dist(self.game.checkpoints[self.nextchx]) < 600:
                self.nextchx += 1
                self.nextchx = self.nextchx % len(self.game.checkpoints)
                if self.nextchx == 0:
                    self.turns += 1

        if self.turns == 3:
            raise Exception(str(self.id) + " won")

        
    def collide(self, pod):

        print("collide", self.dist(pod.pos))
        
        u = pod.pos - self.pos
        u = u/np.linalg.norm(u)
     
        v1p = np.dot(self.v, u)*u
        v2p = np.dot(pod.v, u)*u
        p = v2p - v1p

        if np.linalg.norm(p) < 120:
            p *= 120/np.linalg.norm(p)
        
        self.v += p
        pod.v -= p
    
    def checkCollide(self):
        for pod in self.game.pods:
            if pod.id != self.id:
                if self.dist(pod.pos) < 800:
                    self.collide(pod)
                        



class Game():
    
    def __init__(self, npods = 1):
        self.checkpoints = [np.random.randint(1500, 7500, (2,)) for i in range (4)]
        self.pods = [Pod(self) for i in range(npods)]
        self.history = []

    def step(self):
        for pod in self.pods:
            pod.timestep()
            for i in range(Ndt):
                hi = []
                for pod in self.pods:
                    pod.move()                        
                    hi.append((pod.pos.copy(),pod.orientation))
                self.history.append(hi)

    def play(self):
        try:
            for k in range(1000):
                self.step()
        except Exception as e:
            print(e)
